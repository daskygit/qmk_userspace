...
cleanup() {
    ...
    if [[ -z "${FIRST_INIT:-}" ]]; then
        if [[ -d "$build_dir" ]]; then
            gecho "Entering shell so that extra investigation can occur..."
            gecho "Current directory: $(pwd)"
            gecho "Verification directory: $build_dir"
            bash
        fi
    fi
    if [[ -d "$build_dir" ]]; then
    ...
    if [[ -n "${USE_RAMDISK:-}" ]]; then
        while [[ -n "$(mount | grep "/Volumes/RAM")" ]]; do
            errcho "Waiting for $build_dir to unmount..."
            sleep 1
            hdiutil detach /Volumes/RAM || true
            sleep 1
        done
    fi
}
...
build_targets() {
    ...
    pcmd git diff --name-only $TARGET_BRANCH | /opt/homebrew/opt/grep/libexec/gnubin/grep -P '^keyboards' | gsed -e 's@^keyboards/@@g' -e 's@/keymaps/.*$@@g' -e 's@/[^/]*$@@g' | while read kb; do
        find "keyboards/$kb" \( -name 'rules.mk' \) -and -not -path '*/keymaps/*' | while read rules; do
            kb=$(dirname "$rules" | gsed -e 's@^keyboards/@@g')
            gecho "${kb}:default"
        done
    done | sort | uniq
    ...
    #gecho takashiski/namecard2x4/rev1:default takashiski/namecard2x4/rev2:default
...
build_one() {
    ...
    gecho $target
    [[ -d .build ]] || mkdir .build
    make ${target} $reproducible_build_flags 2>&1 | grep -v '⚠' >.build/$(gecho $target | gsed -e 's@/@_@g').log 2>&1
}
...
main() {
    ...
    unset FIRST_INIT
    if [[ -n "${USE_RAMDISK:-}" ]]; then
        diskutil erasevolume APFS 'RAM' `hdiutil attach -nobrowse -nomount ram://67108864`
        ln -s /Volumes/RAM/ "$build_dir"
    else
    ...
    pcmd make distclean
    pcmd rm .git/hooks/*
    pcmd git remote set-url qmk "$QMK_FIRMWARE_REPO"
    pcmd git checkout "$TARGET_BRANCH"
    pcmd git fetch qmk "$TARGET_BRANCH"
    pcmd git clean -xfd
    pcmd git reset --hard
    ...
    cd "$pr_dir"
    pcmd git fetch qmk pull/"$TARGET_PR_NUMBER"/head:pr/"$TARGET_PR_NUMBER"
    pcmd git checkout -b target_pr pr/"$TARGET_PR_NUMBER"
    pcmd git merge --no-edit --quiet $TARGET_BRANCH

    # Generate the list of targets
    targets=$(build_targets | sort | uniq | xargs gecho)
    ...
    local differences=$( (diff -yW 200 --suppress-common-lines "$build_dir/sha1sums_base.txt" "$build_dir/sha1sums_pr.txt" || true) | awk '{print $2}' | gsed -e 's@\.\(hex\|bin\|uf2\)$@@g' | xargs gecho)
    for difference in $differences; do
        { find "$base_dir/.build/obj_$difference" "$pr_dir/.build/obj_$difference" -type f -name '*.i' 2>/dev/null || true; } | while read file; do
            cat "$file" | gsed -e 's@^#.*@@g' -e 's@^\s*//.*@@g' -e '/^\s*$/d' -e 's@qmk_base@qmk_xxx@g' -e 's@qmk_pr@qmk_xxx@g' | clang-format >"$file.formatted"
        done
    done

    gecho
    gecho '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
    gecho '@@ Listing builds that failed on the base revision:           @@'
    gecho '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
    { cat "$build_dir/mass_compile_base.log" | grep ERR; } || true

    gecho
    gecho '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
    gecho '@@ Listing builds that failed on the PR:                      @@'
    gecho '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
    ...
    gecho
    gecho '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
    gecho '@@ Listing builds that differ between the base and target PR: @@'
    gecho '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
    gecho '@@ Begin @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
    gecho
    ...
    elf_files=$( (diff "$build_dir/sha1sums_base.txt" "$build_dir/sha1sums_pr.txt" || true) | awk '/(hex|bin|uf2)/ {print $3}' | sort | uniq | gsed -e 's@\(hex\|bin\|uf2\)@elf@g' || true)
